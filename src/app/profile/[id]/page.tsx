...
export default async function ProfilePage({
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('id, display_name, university_email, created_at, is_hidden')
    .eq('id', id)
  ...
  for (const season of seasons ?? []) {
    ...
    if (!sRank) continue
    if (sRank.rank === 1) badges.push({ label: '1st', color: 'border-amber-400 text-amber-400', season: season.name })
    else if (sRank.rank === 2) badges.push({ label: '2nd', color: 'border-zinc-400 text-zinc-400', season: season.name })
    else if (sRank.rank === 3) badges.push({ label: '3rd', color: 'border-amber-700 text-amber-700', season: season.name })
  }
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 mt-1">
              {profile.university_email && <Badge variant="chill">university</Badge>}
